...
namespace Log_in
{
    public partial class GoodsReceived : Form
    {
        ...
        private void grd_Click(object sender, EventArgs e)
        {
            ...
            priceTXT.Text = grd.CurrentRow.Cells[4].Value.ToString();
            slTXT.Text = grd.CurrentRow.Cells[5].Value.ToString();
            TXTBrand.Text = grd.CurrentRow.Cells[6].Value.ToString();
        ...
        private void saveBtn_Click(object sender, EventArgs e)
        {
            ...
            if (dk == 1)//Add
            {
                ...
                //Insert into
                sql = "insert into goods values ('" + idTXT.Text + "','" + nameTXT.Text + "','" + dtp.Value.ToShortDateString() + "', '" + spTXT.Text + "', " + priceTXT.Text + "," + slTXT.Text + ", '" + TXTBrand.Text + "')";
                cm = new SqlCommand(sql, cn);
                cm.ExecuteNonQuery();
            }
            else //dk =2
            {
                //Update
                sql = "update goods set nameG = '" + nameTXT.Text + "', dateRe = '" + dtp.Value.ToShortDateString() + "', supplier = '" + spTXT.Text + "', price = " + priceTXT.Text + ", quantity = " + slTXT.Text + ", brand = '" + TXTBrand.Text + "' where ID = '" + idTXT.Text + "'";
                cm = new SqlCommand(sql, cn);
                cm.ExecuteNonQuery();
            }
            formload();
